...
//      document.add(getParagraph("Дата проведения экзамена " + Utils.getDateString(new Date()),
   ...
   * @throws DocumentException при ошибке создания PDF документа
   * @Throws IllegalArgumentException при невозможности определить семестр и курс
   * по указанному году и месяцу, а также если вместо группы null
   */
  private byte[] getMonthMarksReport(final StudyGroup group, 
    final int year, final int month, final boolean empty) throws DocumentException {
    if (null == group) {
      throw new IllegalArgumentException("Группа не должна быть NULL!");
    }
    ...
    if ((0 == course) || (0 == semester)) {
      throw new IllegalArgumentException("Такой комбинации месяца и года не соответствует ни один семестр!");
    }
    // создадим новый лист с размерами A4 и отступами слева и справа по 5 мм, а сверху и снизу - по 10
    Document document = new Document(PageSize.A4, getPt(5), getPt(5), getPt(10), getPt(10));
    PdfWriter writer = PdfWriter.getInstance(document, stream);
  ...
  private byte[] getSemesterConsolidatedReport(final StudyGroup group, final int course, final int semester) {
    throw new InternalServerErrorException("Methor not implemented!");
  }
}
